#include <math.h>
#include <stdio.h>
#include <stdlib.h> // Añadido para 'abort' y 'calloc'
...
void krigea_gik_1(int nvec, float *zestim, float *sigma, float xx, float yy);
float estruc_gki_1(float h);
float monomio(float ax, float ay, int i);
float covmodel_1(float h, int i);
void escribe_gki(int nvec, float *ponde, int ix);
/* ------------------------------- */
...
int main() {
  ...
  for (ix = 0; ix < ntot; ix++) {
    ...
    if (busca(nvec + 1) != 1) {
      krigea_gik_1(nvec, &zestim, &sigma, xx, yy);
      if (zestim > 0.)
        escribe_gki(nvec, vec, ix);
    };
    printf("\rix= %d", ix);
  };
...
int gaussj(float *a, float *y, int ne, int ns)
{
  float *(*ap), val, *tp, *(*yp);
  int i, j, k, piv;
  // ... (resto de la función gaussj sin cambios)
...
void krigea_gik_1(int nvec, float *zestim, float *sigma, float xx,
                float yy)
{
  float *(*a), *b, *bb, c0;
  int i, ii, j, jj, dim;
  // ... (resto de la función krigea_gik_1 sin cambios)
  dim = nvec + nmon[nork];
  ...
  for (i = 0; i < (nvec - 1); i++) {
    ...
    for (j = i + 1; j < nvec; j++) {
      jj = neig[j];
      a[i][j] =
          estruc_gki_1((float)(sqrt(1. * ((x[ii] - x[jj]) * (x[ii] - x[jj]) +
                                        (y[ii] - y[jj]) * (y[ii] - y[jj])))));
      a[j][i] = a[i][j];
    };
  };
...
void escribe_gki(int nvec, float *ponde, int ix)
{
  ...
  // ... (resto de la función escribe_gki sin cambios)
  sq = 1.;
  for (i = nvec; i >= 1; i--) {
    ponde[i] = (-ponde[i - 1]);
    sq += ponde[i] * ponde[i];

...
void krigea_gik(int nvec, float *zestim, float *sigma, float xx, float yy);
...
int main() {
  ...
  printf("CROSVAL PROGRAM: cross-validation of models\n");
  printf("\n Data filename:->");
  scanf("%s", file);
...
void krigea_gik(int nvec, float *zestim, float *sigma, float xx,
                float yy)
{
  float *(*a), *b, *bb, c0;
  int i, ii, j, jj, dim;

# libGCK2.c Geostatistics Library Documentation

The `libGCK2.c` library implements a suite of functions for geostatistical estimation, primarily focusing on **Universal Kriging (UK)** using **Generalized Covariance (GC)** functions and the **Generalized Increment of order k (GIK)** method. The library is typically compiled as a C shared library for use in higher-level languages like Python.
